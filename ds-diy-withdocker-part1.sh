...
if [ "$str1" = "5.4" ];then
  ...
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver419}_dev_dscaodocker.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver419}_uefi-gpt_dev_dscaodocker.img.gz
elif [ "$str1" = "4.14" ];then
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver414}_dev_dscaodocker.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver414}_uefi-gpt_dev_dscaodocker.img.gz
elif [ "$str1" = "5.10" ];then
   mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver510}_dev_dscaodocker.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver510}_uefi-gpt_dev_dscaodocker.img.gz
elif [ "$str1" = "5.15" ];then
...
case $num1 in
  ...
  echo -e "\033[32m >>>正在准备保留配置升级，请稍后，等待系统重启…-> \033[0m"
	...
	if [ ! -d /sys/firmware/efi ];then
		gzip -d openwrt_x86-64-${new_version}_dev_dscaodocker.img.gz
		sysupgrade -n  /tmp/openwrt_x86-64-${new_version}_dev_dscaodocker.img
	else
		gzip -d openwrt_x86-64-${new_version}_uefi-gpt_dev_dscaodocker.img.gz
		sysupgrade -n  /tmp/openwrt_x86-64-${new_version}_uefi-gpt_dev_dscaodocker.img
	fi
    ;;
    *)
	  echo
...
if [ ! -d /sys/firmware/efi ];then
	if [ "$current_version" != "$cloud_version" ];then
		...
		gzip -d /tmp/openwrt_x86-64-${new_version}_dev_dscaodocker.img.gz
		sysupgrade /tmp/openwrt_x86-64-${new_version}_dev_dscaodocker.img
	else
		echo -e "\033[32m 本地已经是最新版本，还更个鸡巴毛啊… \033[0m"
		echo
	...
	if [ "$current_version" != "$cloud_version" ];then
		...
		gzip -d /tmp/openwrt_x86-64-${new_version}_uefi-gpt_dev_dscaodocker.img.gz
		sysupgrade /tmp/openwrt_x86-64-${new_version}_uefi-gpt_dev_dscaodocker.img
	else
		echo -e "\033[32m 本地已经是最新版本，还更个鸡巴毛啊… \033[0m"
		echo
