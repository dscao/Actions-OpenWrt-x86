...
touch files/usr/share/dscao-auto.sh
...
if [ "$str1" = "5.4" ];then
  ...
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver419}_dev_dscao.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver419}_uefi-gpt_dev_dscao.img.gz
elif [ "$str1" = "4.14" ];then
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver414}_dev_dscao.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver414}_uefi-gpt_dev_dscao.img.gz
elif [ "$str1" = "5.10" ];then
  ...
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver515}_uefi-gpt_dev_dscao.img.gz
   ...
   mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver60}_dev_dscao.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver60}_uefi-gpt_dev_dscao.img.gz
fi
ls bin/targets/x86/64 | grep "_dscao.img" | cut -d - -f 3 | cut -d _ -f 1-2 > wget/op_version1
#md5
...
if [ $? != 0 ]; then
	...
	sed -i '$ a alias dscao="bash /usr/share/Check_Update.sh"' /etc/profile
	exit 0
	EOF
fi
grep "dscao-auto.sh"  package/lean/default-settings/files/zzz-default-settings
if [ $? != 0 ]; then
	sed -i 's/exit 0/ /'  package/lean/default-settings/files/zzz-default-settings
	cat>> package/lean/default-settings/files/zzz-default-settings<<-EOF
	sed -i '$ a alias dscao-auto="bash /usr/share/dscao-auto.sh"' /etc/profile
	exit 0
	EOF
fi
EOOF
...
echo $Firmware_Type > /etc/dscao_firmware_type
echo
...
if [ ! -d /sys/firmware/efi ];then
	if [ "$current_version" != "$cloud_version" ];then
		wget -P /tmp "$DEV_URL" -O /tmp/openwrt_x86-64-${new_version}_dev_dscao.img.gz
...
case $num1 in
  ...
  echo -e "\033[32m >>>正在准备保留配置升级，请稍后，等待系统重启…-> \033[0m"
	...
	sleep 3
	if [ ! -d /sys/firmware/efi ];then
		gzip -d openwrt_x86-64-${new_version}_dev_dscao.img.gz
		sysupgrade /tmp/openwrt_x86-64-${new_version}_dev_dscao.img
	else
		gzip -d openwrt_x86-64-${new_version}_uefi-gpt_dev_dscao.img.gz
	...
	if [ ! -d /sys/firmware/efi ];then
		gzip -d openwrt_x86-64-${new_version}_dev_dscao.img.gz
		sysupgrade -n  /tmp/openwrt_x86-64-${new_version}_dev_dscao.img
	else
		gzip -d openwrt_x86-64-${new_version}_uefi-gpt_dev_dscao.img.gz
		sysupgrade -n  /tmp/openwrt_x86-64-${new_version}_uefi-gpt_dev_dscao.img
	fi
    ;;
    *)
	  echo
...
if [ ! -d /sys/firmware/efi ];then
	if [ "$current_version" != "$cloud_version" ];then
		...
		gzip -d /tmp/openwrt_x86-64-${new_version}_dev_dscao.img.gz
		sysupgrade /tmp/openwrt_x86-64-${new_version}_dev_dscao.img
	else
		echo -e "\033[32m 本地已经是最新版本，还更个鸡巴毛啊… \033[0m"
		echo
